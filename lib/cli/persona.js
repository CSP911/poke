...
- The VERY FIRST line of the file must be a metadata comment on one line:
  // POKE-META {"name":"<kebab-name>","description":"<one line, generic — describe the persona family, not this request>","params":[{"name":"<param>","desc":"<meaning, unit, default>"}],"values":[<the values THIS request needs, in order>],"services":[<"touch" if the persona reads api->touch, else empty>]}
...
async function become(request, wish) {
  ...
  if (catalog.length) {
    ...
    if (m && m.match) {
      ...
      if (entry && fs.existsSync(binPath)) {
        ...
        const srcPath = path.join(CACHE_DIR, `${m.match}.c`)
        const src = fs.existsSync(srcPath) ? fs.readFileSync(srcPath, 'utf8') : ''
        await residents.ensureServices(request, residents.servicesUsed(entry, src))
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    ...
    await request(pparPayload(values.length ? values : [0]))
    await request(timePayload())
    await residents.ensureServices(request, residents.servicesUsed(meta, code))
    const r = JSON.parse(await upload(request, 'PRUN', bin))
