...
/* Upload a binary and run it. kind = 'EXEC' (probe, reply is its output),
 * 'PRUN' (persona, reply is JSON) or 'RSLD' (resident driver, always staged). Small binaries go as one frame; larger ones
...
async function upload(request, kind, bin, timeoutMs = 15000) {
  if (kind !== 'RSLD' && bin.length <= SINGLE_MAX) return request(Buffer.concat([Buffer.from(kind), bin]), timeoutMs)
  ...
  const fin = kind === 'PRUN' ? 'PRST' : kind === 'RSLD' ? 'RSLD' : 'EXRN'
